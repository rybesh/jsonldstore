...
function CouchbaseWritable(options, bucket, graph_key) {
  ...
  this._operation = this._bucket[options.operation || 'set']
}
...
CouchbaseWritable.prototype._do = function(key, o, callback) {
  ...
  if (self._operation.name === 'remove')
    self._operation(key, callback)
  else
    self._operation(key, o, callback)

  self.emit(self._operation.name, o)
}
