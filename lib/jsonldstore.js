...
function load_graph(req, res, next) {
  ...
  parsejsonld.on('context', function(c, scope){
    context = c
    // TODO: respect scope
    req.bucket.add(graph_key + '/context', {'@context':c}, function(err, meta) {
      if (err) {
        req.log.error(err, err.code)
        return next(err)
      }
  ...
  parsejsonld.on('finish', function(){
    ...
    req.bucket.add('graph_name/'+hashurl(graph['@id']), graph_key, 
      function(err, meta) {
        if (err) {
        ...
        req.bucket.add(graph_key, addurls.attachURL(graph), function(err, meta){
          if (err) {
            req.log.error(err, err.code)
            return next(err)
          }
    ...
    remover.on('finish', function() {
      req.bucket.remove('graph_name/'+hashurl(doc['@id']), 
        function(err){
          ...
          req.bucket.remove(graph_key+'/context', 
            function(err){
              if (err && err.code !== couchbase.errors.keyNotFound) {
                req.log.error(err, err.code)
                return next(err)
